...
def compute_summary(csv_path: Path) -> str:
    ...
    return summary
def main():
    parser = argparse.ArgumentParser(description="Summarize Dice and HD95 from pipeline A results.")
    parser.add_argument(
      "--csv",
      type=Path,
      required=True,
      help="Path to pipeline_a_test_results_seed42.csv",
    )
    args = parser.parse_args()
    print(compute_summary(args.csv))
