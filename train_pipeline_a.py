#!/usr/bin/env python3
"""
Training script for Pipeline A - Original U-Net with standard configuration
"""
import argparse
import os
import pathlib
...
parser = argparse.ArgumentParser(description='Pipeline A Training - Standard U-Net')
...
parser.add_argument('--pipeline', default='A', help='Pipeline identifier')


def main(args):
    """Main training function for Pipeline A"""
    # Setup
    ngpus = torch.cuda.device_count()
    if ngpus == 0:
        raise RuntimeWarning("This will not be able to run on CPU only")

    print(f"Training Pipeline A with {ngpus} GPUs")
    if args.optim.lower() == "ranger":
        args.warm = 0

    current_experiment_time = datetime.now().strftime('%Y%m%d_%T').replace(":", "")
    args.exp_name = f"PipelineA_{'debug_' if args.debug else ''}{current_experiment_time}_" \
    ...
    args.save_folder = pathlib.Path(f"./runs/pipeline_a/{args.exp_name}")
    ...
    # Create model
    print(f"Creating Pipeline A model: {args.arch}")
    ...
    print(f"Pipeline A - total number of trainable parameters {count_parameters(model)}")
    ...
    if not args.resume:
        for epoch in range(args.warm):
            ...
            print(f"Pipeline A - Train Epoch done in {te - ts} s")
    ...
    print("Starting main Pipeline A training...")
    ...
    for epoch in range(args.start_epoch + args.warm, args.epochs + args.warm):
        try:
            ...
            if (epoch + 1) % args.val == 0 and not args.full:
                ...
                if validation_loss < best:
                    ...
                    save_checkpoint_best(
                        ...
                        save_folder=args.save_folder, )
                    # persist best summary for comparison across resumes
                    best_summary = {
                        "epoch": int(epoch),
                ...
                ts = time.perf_counter()
                print(f"Pipeline A - Val epoch done in {ts - te} s")
            ...
            print("Stopping Pipeline A training loop, doing benchmark")
            break
...
def step(data_loader, model, criterion, metric, deep_supervision, optimizer, epoch, writer, scaler=None,
    ...
    progress = ProgressMeter(
        ...
        prefix=f"Pipeline A {mode} Epoch: [{epoch}]")
